...
describe('SoldierService', () => {
  ...
  beforeEach(() => {
    repo = {
      createSoldier: vi.fn().mockResolvedValue(MOCK_SOLDIER),
      updateSoldier: vi.fn().mockResolvedValue(undefined),
  ...
  it('create() calls repo.createSoldier and logs history', async () => {
    ...
    expect(repo.createSoldier).toHaveBeenCalledWith(input)
  ...
  it('updateStatus() sets Active status', async () => {
    ...
    expect(repo.updateSoldier).toHaveBeenCalledWith({ id: '1234567', status: 'Active', inactiveReason: '' })
  })
  ...
  it('updateStatus() sets Inactive status with reason', async () => {
    ...
    expect(repo.updateSoldier).toHaveBeenCalledWith({ id: '1234567', status: 'Inactive', inactiveReason: 'Medical leave' })
  })

  it('updateFields() calls repo.updateSoldier and logs history', async () => {
    await service.updateFields('1234567', { firstName: 'New', lastName: 'Name', role: 'Medic' as const }, 'admin@test.com')
    expect(repo.updateSoldier).toHaveBeenCalledWith({ id: '1234567', firstName: 'New', lastName: 'Name', role: 'Medic' })
  ...
  it('updateFields() uses newId for history entity when ID is changed', async () => {
    ...
    expect(repo.updateSoldier).toHaveBeenCalledWith({ id: '1234567', newId: '9999999' })
