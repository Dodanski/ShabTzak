...
const HEADER_ROW = [
  'ID', 'First Name', 'Last Name', 'Role', 'Phone', 'Unit', 'ServiceStart', 'ServiceEnd',
...
describe('SoldierRepository', () => {
  ...
  describe('create()', () => {
    ...
    it('writes header row first when sheet is completely empty', async () => {
      ...
      expect(updateSpy).toHaveBeenCalledWith(SHEET_ID, 'Soldiers!A1:P1', [HEADER_ROW])
      expect(appendSpy).toHaveBeenCalledOnce()
    })
  ...
  describe('tabPrefix', () => {
    it('uses prefixed tab name when tabPrefix is provided', async () => {
      ...
      expect(getSpy).toHaveBeenCalledWith(SHEET_ID, 'Alpha_Company!A:P')
    })
  })
})
