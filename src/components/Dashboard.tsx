    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <button
          ...
          className="w-full sm:w-auto px-4 py-3 sm:py-2 bg-olive-700 text-white text-sm rounded-lg hover:bg-olive-800"
        >
          Generate Schedule
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        <div className="bg-white rounded-lg border border-olive-200 shadow-sm p-3 sm:p-4">
          <p className="text-xs sm:text-sm text-olive-500">Active Soldiers</p>
          <p className="text-xl sm:text-2xl font-bold text-olive-800">{activeSoldiers.length}</p>
        </div>
        <div className="bg-white rounded-lg border border-olive-200 shadow-sm p-3 sm:p-4">
          <p className="text-xs sm:text-sm text-olive-500">Pending Requests</p>
          <p className="text-xl sm:text-2xl font-bold text-olive-800">{pendingRequests.length}</p>
        </div>
        <div className="bg-white rounded-lg border border-olive-200 shadow-sm p-3 sm:p-4">
          <p className="text-xs sm:text-sm text-olive-500">Task Assignments</p>
          <p className="text-xl sm:text-2xl font-bold text-olive-800">{taskAssignments.length}</p>
        </div>
        <div className={`rounded-lg border border-olive-200 shadow-sm p-3 sm:p-4 ${conflicts.length > 0 ? 'bg-red-50' : 'bg-white'}`}>
          <p className="text-xs sm:text-sm text-olive-500">Conflicts</p>
          <p className={`text-xl sm:text-2xl font-bold ${conflicts.length > 0 ? 'text-red-600' : 'text-olive-800'}`}>
            {conflicts.length}
          </p>
        </div>
      </div>
      ...
      {soldiers.length > 0 && (
        <div className="bg-white rounded-lg border border-olive-200 shadow-sm p-3 sm:p-4">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-6 text-sm text-olive-600">
